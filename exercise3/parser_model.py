...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # # self.embed_to_hidden_weight = nn.Parameter(torch.zeros(self.embed_size, self.hidden_size)) # nn.init.xavier_uniform_
        # self.embed_to_hidden_weight = nn.Parameter(n_features * self.embed_size, self.hidden_size)
        # nn.init.xavier_uniform_(self.embed_to_hidden_weight)
        # # self.embed_to_hidden_bias = nn.Parameter(torch.ones(self.embed_size))
        # self.embed_to_hidden_bias = nn.Parameter(n_features * 1, self.hidden_size)
        # nn.init.uniform_(self.embed_to_hidden_bias)
        # self.dropout = nn.Dropout(p = dropout_prob)
        # self.hidden_to_logits_weight = nn.Parameter(hidden_size - 1, n_classes)
        # nn.init.xavier_uniform_(self.hidden_to_logits_weight)
        # self.hidden_to_logits_bias = nn.Parameter(1, n_classes)
        # nn.init.uniform_(self.hidden_to_logits_bias)

        # code taken from here: https://github.com/xixiaoyao/CS224n-winter-together/blob/master/Assignments/assignment3/BobOfRivia/parser_model.py
        self.embed_to_hidden_weight = nn.Parameter(
            nn.init.xavier_uniform_(torch.empty(self.embed_size * n_features, hidden_size)))
    ...
    def embedding_lookup(self, w):
        ...
        # For each index `i` in `w`, select `i`th vector from self.embeddings
        # [for i, w_i in enumerate(w)]
        # indexes = torch.index(w, w.shape)

        # x = self.embeddings(w)
        # x = x.view(x.size()[0], -1)
        x = torch.tensor(
            [torch.index_select(self.embeddings, 0, w[i]).view(-1).detach().numpy() for i in range(w.shape[0])])
    ...
    def forward(self, w):
        ...
        # copied code from: https://github.com/ZacBi/CS224n-2019-solutions/blob/master/assignments/a3/parser_model.py

        # TODO: to divide to weight and bias:
        ...
        # drop out layer - done by me unlike the copied code
        dropout = self.dropout(hidd)
