...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            # word_to_be_moved = self.stack[-2]
            # self.stack = self.stack[:-2] + [self.stack[-1]]
            # self.dependencies.append((self.stack[-1], word_to_be_moved))
            self.dependencies.append((self.stack[-1], self.stack[-2]))
            del self.stack[-2]
        elif transition == "RA":
            assert len(self.stack) > 1
            # word_to_be_moved = self.stack[-1]
        ...
        # return self.stack, self.buffer, self.dependencies
...
def minibatch_parse(sentences, model, batch_size):
    ...
    # I have a bug here - the point is I need to split the data into sections, and I move to the next section
    # only when I am done with all the sentences in this section.
    # this way it won't be mixed up in the end
    # partial_parses = [PartialParse(sentence) for sentence in sentences]
    # dependencies = [[] for _ in range(len(sentences))]
    # unfinished_parses = partial_parses[:]
    # num_removed = 0
    # minibatch = unfinished_parses[:min(batch_size, len(sentences))]
    # last_idx = len(minibatch)
    #
    # while num_removed < len(sentences):
    #     minibatch += unfinished_parses[last_idx:min(num_removed + last_idx, len(sentences))]
    #     last_idx += num_removed
    #     len_before = len(minibatch)
    #     minibatch_predicted = model.predict(minibatch)
    #     _dependencies = [m.parse_step(mp) for m, mp in zip(minibatch, minibatch_predicted)]
    #     dependencies[num_removed:num_removed + len(minibatch)] = _dependencies
    #     minibatch = [m for m in minibatch if not m.is_finished()]
    #     num_removed += len_before - len(minibatch)
