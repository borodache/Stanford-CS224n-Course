...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # X = torch.tensor([self.model_embeddings.source[sp] for sp in source_padded])
        X = self.model_embeddings.source(source_padded)
        X_padded = pack_padded_sequence(X, source_lengths)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X_padded)
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens, batch_first=True)
        # enc_hiddens = enc_hiddens.permute(1, 0, 2)  # This line should be right according to the guiding
        # instructions. However, it looks that there is a mistake in them.
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # Ybar_t = torch.stack(([torch.cat((Y_t, o_prev[i])) for i, Y_t in enumerate(Y_ts)]), dim=0)
        # dec_state, combined_output, e_t = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
        # combined_outputs.append(combined_output)
        # o_prev = combined_output
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # print(torch.unsqueeze(dec_hidden, 2).size())
        e_t = torch.squeeze(torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_hidden, 2)), dim=-1)  # Looks that dim = -1 isn't neccesary but I used due to the hint
        # print(e_t.size())
        ### END YOUR CODE
